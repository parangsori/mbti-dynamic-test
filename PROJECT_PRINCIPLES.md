...
- 2026-06-21: 서버 세션 완료 응답에 결과 화면 표시 모델과 결과 기준 시각을 함께 담고, 클라이언트 결과 화면은 해당 값이 있으면 우선 사용하도록 연결했다. 기존 로컬 계산 fallback은 유지해 사용자-facing 흐름 변화 없이 다음 fallback 축소 작업의 기반만 마련했다.
- 2026-06-29: 운영 Production SHA가 `origin/main`의 `d723113`과 일치함을 확인했고, 인증된 admin 지표에서 오늘 1/1회 및 7일 8/8회 시작·완료, 두 기간 모두 서버 세션 fallback 0건·오류 0건을 확인해 관찰 구간을 종료했다. `dev`에서는 서버 완료 요청 실패 시 로컬 결과를 만들지 않고 마지막 질문 상태를 복원해 재응답하도록 첫 fallback 축소를 구현했으며, 강제 `503` 브라우저 시나리오와 기본 회귀 검증을 통과했다. `main` 반영 전에는 dev 프리뷰의 모바일 흐름을 확인한다.
- 2026-06-30: PWA를 닫았다 다시 열었을 때 완료 결과가 계속 첫 화면을 차지하지 않도록 `pendingResult`의 역할을 비정상 종료 복구 구간으로 한정했다. 복구 결과가 로컬 기록에 실제 저장된 뒤에만 pending을 제거하며, 저장 실패 시에는 pending을 유지하고 오류 진단을 남긴다. 이전 형식의 pending 데이터도 저장 시각을 안정적인 기록 키로 사용해 중복 기록 없이 한 번 복구되고, 그다음 재실행부터 시작 화면이 표시되도록 검증했다.
- 2026-06-30: 저사양 Android Chrome에서 매 문항 전환이 무겁게 느껴지는 문제를 v1.7.11에서 개선했다. 답변 확정 대기를 800ms에서 420ms로 줄이고, 문항마다 전체 `QuestionView`를 다시 마운트하던 구조를 안정적인 화면 컨테이너와 짧은 카드 전환으로 바꿨다. 문항 진행바는 transform 기반으로 움직이고 문항 단계에서는 대형 blur blob의 반복 이동을 멈춰, 기존 무드와 선택 피드백을 유지하면서 반복 페인트 부담을 줄였다. 모바일 실브라우저에서 기본 12문항, 보정 2문항, 결과 진입, 이전 문항 복원과 오류 로그 없음까지 확인했다.
- 2026-06-30: 첫 실행에서 분석 안내 캐릭터와 결과 타입 캐릭터가 늦게 보이는 문제를 완화했다. 분석 캐릭터는 세션 시작 직후 미리 받아두고, 최종 캐릭터는 마지막 답변에서 결과가 정해지는 즉시 결과 화면 코드와 병렬로 받아 분석 대기 시간 동안 디코딩한다. 비정상 종료 뒤 `pendingResult`를 복구하는 PWA 실행에서도 부팅 중 같은 준비를 시작한다. 캐릭터 경로는 가벼운 공용 매핑으로 분리해 초기 번들에 전체 캐릭터 설명 데이터가 합쳐지지 않도록 유지했으며, 모바일 실브라우저에서 분석·결과 진입 시 두 이미지 모두 512×512 디코딩 완료 상태이고 오류 로그가 없음을 확인했다.
- 2026-06-30: 문항 전환 모션과 진행률의 의미를 회귀 방지 기준으로 고정했다. 탭·키보드 선택은 실제 스와이프처럼 카드를 화면 밖으로 날리지 않고, 문항 본문 교체에도 좌우 이동을 중첩하지 않는다. 실제 스와이프 퇴장과 첫 문항의 작은 안내 떨림만 별도 경로로 유지한다. 상단 진행 막대와 점 표시는 현재 문항을 미리 완료로 세지 않고 선택이 확정된 답변 수만 반영하며, 점 슬롯은 고정 폭으로 두어 문항마다 주변 점이 재배치되지 않게 한다.
