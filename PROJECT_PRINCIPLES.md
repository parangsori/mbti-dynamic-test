...
- 2026-06-16: 서비스 디자인 개편 Phase 2로 문항·보정 화면의 진행 정보와 질문 카드 위계를 단순화한 v1.7.6을 준비했다. 문항 상단 이미지는 타입 캐릭터로 재제작하지 않고 기존 맥락 이미지를 유지하되, 무드 컬러·유리 질감·테두리 톤만 결과 화면 방향과 맞춘다. 점수 계산, 문항 선택, 보정 판정, 탭·스와이프·키보드 입력 로직은 변경하지 않는다.
