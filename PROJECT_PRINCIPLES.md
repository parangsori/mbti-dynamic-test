...
- 2026-07-01: 문항 전환과 선택 모션은 `main`에서 검증된 좌우 전환·스와이프·탭 확정 반응을 기준으로 유지한다. 상단 진행 막대와 점 표시는 이 모션과 분리해 현재 문항을 미리 완료로 세지 않고, 기본·보정 문항 모두 선택이 확정된 답변 수만 반영한다. 점 슬롯은 고정 폭으로 두어 진행 시 주변 점이 재배치되지 않게 한다.
