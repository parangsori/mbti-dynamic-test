...
- PostHog 이벤트 수집: 메인 사용자 서비스는 `phc_` 프로젝트 토큰과 `https://e.beatblue.net` reverse proxy를 사용합니다.
...
- `phx_` Personal API Key는 admin 서버 전용입니다. 메인 사용자 서비스의 `VITE_POSTHOG_KEY`에는 절대 넣지 않습니다.
- 메인 사용자 서비스의 `VITE_POSTHOG_KEY`에는 `phc_` Project token만 사용합니다.
...
메인 사용자 서비스 Vercel 프로젝트에는 아래 값을 등록합니다.

```env
VITE_POSTHOG_KEY=phc_your_posthog_project_token
...
6. 메인 사용자 서비스의 PostHog reverse proxy `e.beatblue.net`이 Live 상태인지 확인합니다.
7. PostHog Installation Health가 6/6 passed인지 확인합니다.
